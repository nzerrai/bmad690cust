...
Annotation `@Auditable` sur toutes les méthodes de génération + export. `AuditTrailAspect` (`@AfterReturning`) logue en table `audit_logs` : `user_id`, `timestamp_utc`, `action`, `dataset_id`, `volume`. Transparent pour le code métier.
...
| 4 | Toute génération de données déclenche `@Auditable` | D-06 |
